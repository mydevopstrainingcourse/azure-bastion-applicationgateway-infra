...
module "public_ip_gateway" {
  ...
  source                  = "../../modules/azurerm_public_ip"
  public_ip_name          = "appgtw-pip"
  resource_group_location = var.resource_group_location
  resource_group_name     = var.resource_group_name
}

module "applicationgateway" {
  depends_on           = [module.resource_group, module.virtual_machine, module.public_ip_gateway, module.subnet]
  source               = "../../modules/azurerm_application_gateway"
  appgtw_name          = "Ankit-App-Gateway"
  appgtw_location      = var.resource_group_location
  resource_group_name  = var.resource_group_name
  appgtw_public_ip     = "appgtw-pip"
  subnet_name          = "AzureApplicationGatewaySubnet"
  virtual_network_name = var.virtual_network_name

}

module "appgtw_nic_association" {
  for_each            = var.nic_config
  depends_on          = [module.resource_group, module.virtual_machine, module.applicationgateway, module.public_ip_gateway]
  source              = "../../modules/azurerm_nic_appgtw_association"
  resource_group_name = var.resource_group_name
  nic_name            = each.value.nic_name
  appgtw_name         = "Ankit-App-Gateway"
